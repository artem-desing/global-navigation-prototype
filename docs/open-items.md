...
- **Sigil set for the Palette.** GitLab uses `:` projects, `@` users, `~` files, `>` commands. Wallarm equivalent: `:` Products, `@` entities by Resource ID (`dataplane-id`, `route-id`, `policy-id`), `~` settings, `>` actions? Confirm or adjust.
